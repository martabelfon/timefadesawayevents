...
export default function NoticiaPage() {
  ...
  if (!noticia) return <div>{t('errorMessage') || 'Noticia no encontrada'}</div>;
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 text-gray-900">
      ...
      <section className="max-w-5xl mx-auto pt-[120px] pb-20 px-4 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col gap-4 mt-4">
          {["descripcion", "descripcion1", "descripcion2", "descripcion3", "descripcion4", "descripcion5"]
            .map((descKey) => {
              const value = safeGet((noticia as any)[descKey], lang);
              if (!value) return null;
              return (
                <p key={descKey} className="text-white text-lg mb-2 text-justify" dangerouslySetInnerHTML={{ __html: value }} />
              );
            })}
        </div>
      </section>
